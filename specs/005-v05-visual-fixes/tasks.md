...
- [X] T001 Establish baseline: run `npm run typecheck && npm run lint && npm run test:run`, then `npm run build`; record the current gzipped JS bundle size for the later delta report.
...
- [X] T002 [P] [US1] Add failing test in `tests/integration/spinSettle.test.tsx` (new) asserting that after a spin completes (animation on, and animation off), the grid renders the freshly-spun symbols (`state.magicGrid`) and never an all-blank/placeholder column.

### Implementation for User Story 1

- [X] T003 [US1] Update `displayColumns` in `src/components/SlotGrid.tsx` to source the settle target from `state.magicGrid` whenever it is present (during `spinning` and `magic`), falling back to `lastSpinResult` then the placeholder.
- [X] T004 [US1] Run `tests/integration/spinSettle.test.tsx`; confirm green and visually confirm no empty-cell frame on first spin and subsequent spins.
...
- [X] T005 [P] [US2] Add failing test in `tests/unit/ReelColumn.test.tsx` (new) asserting that `ReelColumn` updates its rendered icons when the `icons` prop changes while `spinning` is false.
- [X] T006 [P] [US2] Extend `tests/integration/magicPhase.test.ts` to assert the rendered grid text changes after `MAGIC_RESPIN` and after `MAGIC_SWAP` (not only the reducer state).

### Implementation for User Story 2

- [X] T007 [US2] Add an effect in `src/components/ReelColumn.tsx` that syncs `displayIcons` to the `icons` prop when it changes and no animation is running (do not stomp an in-flight spin).
- [X] T008 [US2] Run the US2 tests (T005, T006); confirm green.
...
- [X] T009 [US3] Extend `tests/unit/ReelColumn.test.tsx` with a failing test: a respin trigger runs the column animation when `animate` is true and settles on the new icons; with `animate` false it updates immediately and does not animate.

### Implementation for User Story 3

- [X] T010 [US3] Add a per-column respin animation path to `src/components/ReelColumn.tsx` (e.g., a `respinToken` prop that runs the existing 200 ms interval shuffle once with a single stop timeout, honoring the `animate` flag).
- [X] T011 [US3] Wire the trigger: pass a per-column respin trigger from `src/components/SlotGrid.tsx`, and in `src/App.tsx` make the respin action both trigger that column's animation and dispatch `MAGIC_RESPIN`.
- [X] T012 [US3] Run US3 tests; confirm only the targeted column animates and the animation-off path is immediate.
...
- [X] T013 [US4] Extend `tests/unit/ReelColumn.test.tsx` with a failing test asserting a locked column renders a clear, persistent locked indicator (queryable element/label) distinct from unlocked columns.

### Implementation for User Story 4

- [X] T014 [US4] Strengthen the locked-column indicator in `src/components/ReelColumn.tsx` (padlock badge + distinct border/label) within the existing 48×48 px bounding boxes (no layout shift).
- [X] T015 [US4] Run US4 test; confirm green; manual check at 720×1280 px.
...
- [X] T016 [US5] Extend `tests/integration/magicPhase.test.ts` with a failing test asserting that when a column-targeting mode (respin/lock) is active, each column renders an emphasized click target/affordance that is absent when no such mode is active.

### Implementation for User Story 5

- [X] T017 [US5] Add a clear column click-target affordance (shown only when respin/lock mode is active, with hover/focus emphasis) across `src/components/SlotGrid.tsx` and `src/components/ReelColumn.tsx`, keeping within the existing column footprint.
- [X] T018 [US5] Run US5 test; confirm green; manual mis-click check.
...
- [X] T019 [P] [US6] Update `tests/unit/catalog.test.ts` to expect costs air `{copper,1}`, water `{copper,1}`, earth `{silver,1}` (fails before impl).
- [X] T020 [P] [US6] Add `tests/unit/market.test.tsx` (new) asserting market items render in ascending normalized-price order (cheapest first).

### Implementation for User Story 6

- [X] T021 [P] [US6] Update `src/game/catalog.ts` costs: `air → { currency: 'copper', amount: 1 }`, `water → { currency: 'copper', amount: 1 }`, `earth → { currency: 'silver', amount: 1 }`.
- [X] T022 [US6] Add an ascending normalized-price sort in `src/components/Market.tsx` (tier weights copper=1, silver=100, gold=10000, mirroring the 100:1 auto-convert ratios).
- [X] T023 [US6] Run US6 tests; confirm green.
...
- [X] T024 [P] [US7] Add/extend `tests/unit/initialState.test.ts` (new) asserting a fresh game has deck = 1 air, 1 water, 1 apple, 1 copper and currencies air=10, water=10, food=100 (fails before impl).

### Implementation for User Story 7

- [X] T025 [P] [US7] Update `startingAmount` in `src/game/currencyRegistry.ts`: `air` → 10, `water` → 10 (food already 100; others unchanged).
- [X] T026 [P] [US7] Update `reel.icons` in `src/game/initialState.ts` to `[air, water, apple, copper]`.
- [X] T027 [US7] Run US7 tests; confirm green; confirm existing `version: 4` saves still load (no migration) and keep their stored balances.
...
- [X] T028 [US9] Extend `tests/integration/magicPhase.test.ts` with failing tests: clicking a guide row sets the active mode (selected styling), clicking the active row clears it, clicking another row switches it, an unaffordable row is not selectable, the swap "select 2nd cell" hint appears in the guide, and the old toggle button strip is no longer rendered.

### Implementation for User Story 9

- [X] T029 [US9] Lift `magicMode` (and `swapFrom`) state out of `src/components/SlotGrid.tsx` into `src/App.tsx`; pass `magicMode` + `onSelectMode` to `MagicPhasePanel` and `magicMode` to `SlotGrid`.
- [X] T030 [US9] Make each ability row in `src/components/MagicPhasePanel.tsx` a clickable selector (selected/disabled states; unaffordable rows non-selectable) and host the swap "select 2nd cell" hint.
- [X] T031 [US9] Remove the duplicate toggle button strip from `src/components/SlotGrid.tsx` and consume `magicMode` via props (preserve column/cell click handling for respin/lock/swap/boost).
- [X] T032 [US9] Run US9 tests; confirm a single selector and that grid click-handling still works.
...
- [X] T033 [P] [US8] Add `SET_CURRENCY` cases to `tests/unit/reducer.test.ts` (fail first): sets a value; sets 0; rejects negative / `NaN` / `Infinity` (state unchanged); floors non-integer; unknown currency unchanged; `phase` and other currencies unchanged.

### Implementation for User Story 8

- [X] T034 [P] [US8] Add `| { type: 'SET_CURRENCY'; currency: CurrencyKey; amount: number }` to the `GameAction` union in `src/game/types.ts`.
- [X] T035 [US8] Implement the `SET_CURRENCY` case (with the validation in contracts/reducer-actions.md) in `src/game/reducer.ts`, persisting via `saveState` on valid input.
- [X] T036 [P] [US8] Create `src/components/CheatPanel.tsx` (new): hidden resource editor with per-currency inputs that dispatch `SET_CURRENCY`.
- [X] T037 [US8] Wire a secret trigger to open the cheat panel and dispatch `SET_CURRENCY` in `src/App.tsx`; ensure it is hidden/unreachable during normal play.
- [X] T038 [US8] Run US8 tests; confirm green; manual easter-egg + invalid-input check.
...
- [X] T039 [P] Run the `quickstart.md` manual checks at 720×1280 px (all nine stories).
- [X] T040 Remove any dead code / unused imports introduced by the changes (e.g., the removed toggle strip).
- [X] T041 **[GATE 1] Typecheck** — `npm run typecheck` exits 0 (blocks next gates).
- [X] T042 **[GATE 2] Lint** — `npm run lint` exits 0 with zero errors.
- [X] T043 **[GATE 3] Unit Tests** — `npm run test:unit` passes.
- [X] T044 **[GATE 4] Integration Tests** — `npm run test:integration` passes.
- [X] T045 **[GATE 5] Build** — `npm run build` compiles cleanly.
- [X] T046 **[GATE 6] Bundle Size** — gzipped JS bundle ≤ 250 KB; report the before/after delta in the PR.
